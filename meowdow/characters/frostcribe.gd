...
func _process(delta):
	...
	if player and not is_talking and Input.is_action_just_pressed("interact"):
		# Only interactable after guide intro
...
func start_dialogue():
	...
	var dialog = Dialogic.start("res://Timelines/towns/Aubrialis(Town2/KeeperTalks.dtl")
	...
	Dialogic.timeline_ended.connect(func():
		...
		# First visit → send to Thaw
		if GlobalData.quest_step == 10:
			GlobalData.quest_step = 11  # next: talk to Thaw
			GlobalData.create_save()

		# Second visit (has thawbloom) → unlock Rhollow
		elif GlobalData.quest_step == 14:

			GlobalData.rhollow_unlocked = true
			GlobalData.quest_step = 15  # next: find statue to Rhollow
			GlobalData.create_save()
			print("Rhollow unlocked!")
