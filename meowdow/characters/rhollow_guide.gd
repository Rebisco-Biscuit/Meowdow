...
func start_dialogue():
	...
	var dialog = Dialogic.start("res://Timelines/towns/Rhollow(Town3/NPC Talk.dtl")
	...
	Dialogic.timeline_ended.connect(func():
		...
		# Give Gloomberry (strawberry) seed
			var seed = preload("res://inventory/collectables/strawberry_seed.tscn").instantiate()
			if player and player.inventory and seed.get("item") != null:
				player.inventory.insert(seed.item)
				print("Gloomberry seed given!")
			seed.queue_free()

		GlobalData.quest_step = 17  # next: find Glow
